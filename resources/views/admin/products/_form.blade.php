...
@endphp
<div x-data="productForm({{ \Illuminate\Support\Js::from($variants) }}, {{ \Illuminate\Support\Js::from($features) }})" class="space-y-6">
        ...
        <section class="overflow-hidden rounded-[2rem] border border-white/70 bg-white/85 shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
            ...
            <div class="p-6">
                <div class="grid gap-5 lg:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div class="lg:col-span-2 xl:col-span-4"><label class="mb-2 block text-sm font-medium">Short description</label><textarea name="short_description" rows="3" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">{{ old('short_description', $product['short_description'] ?? '') }}</textarea></div>

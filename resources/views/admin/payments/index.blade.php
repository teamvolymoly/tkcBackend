...
<div class="space-y-6">
    ...
    <form method="GET" class="grid gap-4 rounded-lg border border-white/70 bg-white/80 p-6 shadow-lg shadow-slate-200/40 md:grid-cols-[1fr_180px_180px_auto] dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
        <input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Search order, transaction, customer" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">
        ...
        <input type="text" name="payment_method" value="{{ $filters['payment_method'] ?? '' }}" placeholder="Method" class="rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                ...
                <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-800 dark:bg-slate-900">
                    @foreach ($payments['data'] as $payment)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-right"><a href="{{ route('admin.payments.show', $payment['id']) }}" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold dark:border-slate-700">Inspect</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>

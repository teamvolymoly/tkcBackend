...
<head>
    ...
    <title>Forgot Password | {{ config('app.brand_name') }}</title>
    ...
    <div class="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10">
        ...
        <div class="relative w-full max-w-xl overflow-hidden rounded-lg border border-[#ddd6ca] bg-white shadow-2xl shadow-slate-300/30 dark:border-[#354335] dark:bg-[#171f18] dark:shadow-black/30">
            <div class="p-6 sm:p-10">
                ...
                <p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Enter your administrator email address to receive a six-digit verification code.</p>
                ...
                <form method="POST" action="{{ route('admin.password.email') }}" class="mt-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm outline-none transition placeholder:text-slate-400 focus:border-sky-400 focus:ring-4 focus:ring-sky-100 dark:border-slate-700 dark:bg-slate-950 dark:focus:ring-sky-500/10" placeholder="admin@example.com" required>
                    </div>
                    <button type="submit" class="inline-flex w-full items-center justify-center rounded-lg bg-slate-900 px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-slate-900/15 transition hover:-translate-y-0.5 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">Send OTP</button>
                </form>

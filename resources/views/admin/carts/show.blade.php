...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.carts.index') }}" class="rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold dark:border-slate-700">Back</a>
    </div>
    <div class="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        <section class="rounded-lg border border-white/70 bg-white/80 p-6 shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
            ...
            <dl class="mt-5 space-y-3 text-sm">
                <div class="flex items-center justify-between rounded-lg bg-slate-50 px-4 py-3 dark:bg-slate-950/60"><dt class="text-slate-500">Email</dt><dd class="font-medium">{{ $cart['user']['email'] ?? 'N/A' }}</dd></div>
                <div class="flex items-center justify-between rounded-lg bg-slate-50 px-4 py-3 dark:bg-slate-950/60"><dt class="text-slate-500">Phone</dt><dd class="font-medium">{{ $cart['user']['phone'] ?? 'N/A' }}</dd></div>
                <div class="flex items-center justify-between rounded-lg bg-slate-50 px-4 py-3 dark:bg-slate-950/60"><dt class="text-slate-500">Items</dt><dd class="font-medium">{{ count($cart['items'] ?? []) }}</dd></div>
            </dl>
        </section>
        <section class="rounded-lg border border-white/70 bg-white/80 p-6 shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">

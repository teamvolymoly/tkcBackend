...
<div class="space-y-6">
    ...
    <form method="GET" class="grid gap-4 rounded-lg border border-white/70 bg-white/80 p-6 shadow-lg shadow-slate-200/40 md:grid-cols-[1fr_auto] dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
        <input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Search customer or product" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-4">
                                <form method="POST" action="{{ route('admin.wishlists.destroy', $item['id']) }}" class="flex justify-end" data-confirm="Remove this wishlist item?">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="rounded-lg border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-600 dark:border-rose-500/20 dark:text-rose-300">Remove</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach

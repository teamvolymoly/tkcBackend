            ...
            <div class="p-6 sm:p-10">
                ...
                <form method="POST" action="{{ route('admin.password.otp.verify') }}" class="mt-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="otp" value="{{ old('otp') }}" inputmode="numeric" maxlength="6" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm tracking-[0.35em] shadow-sm outline-none transition placeholder:text-slate-400 focus:border-sky-400 focus:ring-4 focus:ring-sky-100 dark:border-slate-700 dark:bg-slate-950 dark:focus:ring-sky-500/10" placeholder="123456" required>
                    </div>
                    <button type="submit" class="inline-flex w-full items-center justify-center rounded-2xl bg-slate-900 px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-slate-900/15 transition hover:-translate-y-0.5 hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">Verify OTP</button>
                </form>

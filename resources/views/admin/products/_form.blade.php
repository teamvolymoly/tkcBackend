                ...
                <template x-for="(variant, index) in variants" :key="variant.uid">
                    <div x-transition class="space-y-5 rounded-[1.5rem] border border-slate-200 bg-slate-50/70 p-5 dark:border-slate-800 dark:bg-slate-950/60">
                        ...
                        <div class="grid gap-4 xl:grid-cols-12">
                            ...
                            <div class="xl:col-span-1"><label class="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Price</label><input type="number" step="0.01" :name="`variants[${index}][price]`" x-model="variant.price" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-900" required></div><div class="xl:col-span-1"><label class="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Compare Price</label><input type="number" step="0.01" :name="`variants[${index}][compare_price]`" x-model="variant.compare_price" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-900"></div>
                        ...
                        <div class="grid gap-4 xl:grid-cols-[1fr_320px]">
                            <div class="space-y-4 rounded-[1.25rem] border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                                ...
                                <template x-for="(ritual, ritualIndex) in variant.brewing_rituals" :key="ritual.uid"><div class="grid gap-3 rounded-2xl border border-slate-200 p-3 md:grid-cols-2 dark:border-slate-700"><input :name="`variants[${index}][brewing_rituals][${ritualIndex}][group]`" x-model="ritual.group" placeholder="Hot Brew / Iced Brew" class="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950"><input :name="`variants[${index}][brewing_rituals][${ritualIndex}][title]`" x-model="ritual.title" placeholder="Water / Serve" class="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950"><input :name="`variants[${index}][brewing_rituals][${ritualIndex}][icon]`" x-model="ritual.icon" placeholder="cup" class="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950"><input :name="`variants[${index}][brewing_rituals][${ritualIndex}][value]`" x-model="ritual.value" placeholder="200ml" class="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950"><div class="md:col-span-2"><input :name="`variants[${index}][brewing_rituals][${ritualIndex}][text]`" x-model="ritual.text" placeholder="Steep for 3-5 mins" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950"></div><div class="md:col-span-2"><button type="button" @click="removeRitual(index, ritualIndex)" class="rounded-2xl border border-rose-200 px-4 py-3 text-sm font-semibold text-rose-600 dark:border-rose-500/20 dark:text-rose-300">Remove</button></div></div></template>
                            </div>

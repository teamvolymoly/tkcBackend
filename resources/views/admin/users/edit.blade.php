            ...
            <form method="POST" action="{{ route('admin.users.update', $user['id']) }}" class="mt-8 space-y-6" data-loading-form>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $user['name'] ?? '') }}" class="block w-full rounded-lg border-slate-200 bg-white/80 px-4 py-3 text-sm shadow-sm focus:border-sky-500 focus:ring-sky-500 dark:border-slate-700 dark:bg-slate-950/60 dark:text-white" required>
                    </div>
                    <div>
                        <label for="email" class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200">Email address</label>
                        <input id="email" name="email" type="email" value="{{ old('email', $user['email'] ?? '') }}" class="block w-full rounded-lg border-slate-200 bg-white/80 px-4 py-3 text-sm shadow-sm focus:border-sky-500 focus:ring-sky-500 dark:border-slate-700 dark:bg-slate-950/60 dark:text-white" required>
                    </div>
                </div>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input id="phone" name="phone" type="text" value="{{ old('phone', $user['phone'] ?? '') }}" class="block w-full rounded-lg border-slate-200 bg-white/80 px-4 py-3 text-sm shadow-sm focus:border-sky-500 focus:ring-sky-500 dark:border-slate-700 dark:bg-slate-950/60 dark:text-white">
                    </div>
                    <div>
                        <label for="role" class="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-200">Role</label>
                        <select id="role" name="role" class="block w-full rounded-lg border-slate-200 bg-white/80 px-4 py-3 text-sm shadow-sm focus:border-sky-500 focus:ring-sky-500 dark:border-slate-700 dark:bg-slate-950/60 dark:text-white">

        ...
        <section class="rounded-lg border border-white/70 bg-white/80 p-6 shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
            ...
            <form method="POST" action="{{ route('admin.payments.update', $payment['id']) }}" class="mt-5 space-y-4" data-loading-form>
                ...
                <label class="block text-sm font-medium">Payment method
                    <input type="text" name="payment_method" value="{{ old('payment_method', $payment['payment_method']) }}" required class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                </label>
                <label class="block text-sm font-medium">Transaction ID
                    <input type="text" name="transaction_id" value="{{ old('transaction_id', $payment['transaction_id']) }}" class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                </label>
                <label class="block text-sm font-medium">Status
                    <select name="status" class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                ...
                <button type="submit" class="w-full rounded-lg bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">Save Payment</button>
            </form>
        </section>
        <section class="rounded-lg border border-white/70 bg-white/80 p-6 shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none space-y-6">
            <div>

...
<div class="space-y-6">
    ...
    <form method="GET" class="grid gap-4 rounded-lg border border-white/70 bg-white/80 p-6 shadow-lg shadow-slate-200/40 md:grid-cols-[1fr_auto] dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
        <input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Search code" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">
    ...
    @else
        <div class="overflow-hidden rounded-lg border border-white/70 bg-white/80 shadow-lg shadow-slate-200/40 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-none">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.coupons.show', $coupon['id']) }}" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold dark:border-slate-700">View</a>
                                    <a href="{{ route('admin.coupons.edit', $coupon['id']) }}" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold dark:border-slate-700">Edit</a>
                                    <form method="POST" action="{{ route('admin.coupons.destroy', $coupon['id']) }}" data-confirm="Delete this coupon?">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="rounded-lg border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-600 dark:border-rose-500/20 dark:text-rose-300">Delete</button>
                                    </form>
                                </div>
                            </td>
                        </tr>

    ...
    <div class="grid gap-6 p-6 xl:grid-cols-[1.6fr_0.9fr]">
        <div class="space-y-5">
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $post['title'] ?? '') }}" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm" required>
            </div>
            <div>
                <label class="mb-2 block text-sm font-medium">Excerpt</label>
                <textarea name="excerpt" rows="4" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm" placeholder="Short intro for cards and previews">{{ old('excerpt', $post['excerpt'] ?? '') }}</textarea>
            </div>
            <div>
                <label class="mb-2 block text-sm font-medium">Content</label>
                <textarea name="content" rows="16" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm" placeholder="Write the full blog content here..." required>{{ old('content', $post['content'] ?? '') }}</textarea>
            </div>
        </div>
            ...
            <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-5 dark:border-slate-800 dark:bg-slate-950/60">
                ...
                <div class="mt-4 space-y-4">
                    <div>
                        ...
                        <input type="datetime-local" name="published_at" value="{{ $publishedAt }}" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm">
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium">Featured image</label>
                        <input type="file" name="featured_image" accept="image/*" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm">
                    </div>
            ...
            <div class="rounded-lg border border-slate-200 bg-slate-50/70 p-5 dark:border-slate-800 dark:bg-slate-950/60">
                ...
                <div class="mt-4 space-y-4">
                    <div>
                        ...
                        <input type="text" name="meta_title" value="{{ old('meta_title', $post['meta_title'] ?? '') }}" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm">
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium">Meta description</label>
                        <textarea name="meta_description" rows="5" class="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm">{{ old('meta_description', $post['meta_description'] ?? '') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
